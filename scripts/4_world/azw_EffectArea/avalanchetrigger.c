...
{
	protected int m_AvalancheState; // Used to get the state from linked area and update effects
	
	void AvalancheTrigger_dynamic()
	{
	...
	override protected void OnStayClientEvent(TriggerInsider insider, float deltaTime) 
	{
		...
		if ( insider.GetObject().IsMan() )
		{
			...
			if ( playerInsider.IsControlledPlayer() )
			{
				...
				bool nonDefaultState = m_AvalancheState > AvalancheAreaState.LIVE;
				if ( nonDefaultState )
				{
					// if not we check the specific state and update local effect values
					int localPartBirthRate;
					if ( m_AvalancheState == AvalancheAreaState.DECAY_START )

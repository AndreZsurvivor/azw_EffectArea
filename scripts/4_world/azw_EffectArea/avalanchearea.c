class AvalancheArea_base : EffectArea
{
	...
	void PlacePoints( vector pos, float radius, int nbRings, int innerSpacing, bool outerToggle, int outerSpacing, int outerOffset, int partId )
	{
		if (partId == 0)
		{
		...
		float angle = m_Rotation; // rotation parameter passed from cfgeffectarea.json
	...
	override void PlaceParticles( vector pos, float radius, int nbRings, int innerSpacing, bool outerToggle, int outerSpacing, int outerOffset, int partId )
	{
		...
		foreach (vector emitPos: m_EmitterPoints)
		{
			Print(emitPos);
		}
